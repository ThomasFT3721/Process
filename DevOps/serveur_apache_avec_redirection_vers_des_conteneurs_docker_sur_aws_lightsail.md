...
5. Passer en administrateur avec la commande :
    ```shell
    sudo su
    ```
...
8. Supprimer les configurations par défaut du serveur avec les commandes :
    ...
    ```
9. Listé avec `a2query -s`, s'il reste des configurations active, il faut les désactiver.
...
12. Ajouter les configurations avec les commandes :
    ...
    ```
13. Activer les modules de proxy avec les commandes :
    ```shell
    a2enmod proxy
    a2enmod proxy_http
    ```
14. Relancer le serveur avec la commande :
    ```shell
